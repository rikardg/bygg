...
Each `Action` declares its input and output files. Bygg will store the hash digests of these files between builds. If the input and output hashes match, the `Action` can safely be skipped.

This means that we have to declare all relevant input and output files correctly, or at least sufficiently so.
